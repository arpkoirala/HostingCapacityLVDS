...
function mn_data_pf(data, load_data, pv_data, inst_data; time_steps=8760)
    ...
    for (id_s,device) in data["load"]
        ...
        if device["source_id"] in names(load_data)
            ...
            pv_gen=pv_prof.*data["PV"][id_s]["p_size"]
            # print(pv_gen)
            pv_gen= pv_gen./(1e6*power_base)/0.405
...
function mn_data_opf(data, load_data, pv_data, inst_data,time_opf)
    ...
    for (id_s,device) in data["load"]
        ...
        if device["source_id"] in names(load_data)
            ...
            # print(pv_gen)
            pv_max=inst_data[(inst_data[!,"Meter_name"].==device["source_id"]),:][!,"PVmax_building_kWp"]
            ...
            for i in 1:length(time_opf)
                ...
                qd = pd/20
                    ...
                    mn_model["nw"]["$(i)"]["PV"][id_s]["pd"] = pv_gen[time_opf[i]]/3*(1/0.405)
                    mn_model["nw"]["$(i)"]["PV"][id_s]["qd"] = 0
                    mn_model["nw"]["$(i)"]["PV"][id_s]["p_max"] = pv_max[1]
                    mn_model["nw"]["$(i)"]["PV"][id_s]["p_min"] = 0*0.405
                    mn_model["nw"]["$(i)"]["PV"][id_s]["p_no_max"] = pv_max[1]/0.405
            ...
            for i in 1:length(time_opf)
                mn_model["nw"]["$(i)"]["PV"][id_s]["p_max"] = 0*0.405
                mn_model["nw"]["$(i)"]["PV"][id_s]["p_min"] = 0*0.405
...
function add_PV_from_case(data,case_data,case)

    for (id_s,device) in data["load"]
        if device["source_id"] in case_data[!,"Metername"]
            data["PV"][id_s]["p_size"]=case_data[(case_data[!,"Metername"].==device["source_id"]),:][!,case][1]
        end
    end
return data
end
